...
select subscription_status,avg(purchase_amount) as average_spend,sum(purchase_amount)as total_revenue from customer 
group by subscription_status order by average_spend,total_revenue desc;

#Which 5 products have the highest Amount of purchases with discounts applied?
select item_purchased,sum(purchase_amount) from customer where discount_applied="yes"
 group by item_purchased order by sum(purchase_amount) desc limit 5;
